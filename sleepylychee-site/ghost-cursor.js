...
export function setupGhostCursor(host, options = {}) {
  ...
  const fadeDuration = fadeDurationMs ?? 1500;
  ...
  // Smaller halo: radius scaled down from the original (0.5 + 0.3/iScale).
  const fragmentShader = `
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      float radius = 0.18 + 0.08 * (1.0 / iScale);
  ...
  function resize() {
    ...
    if (cssW <= 0 || cssH <= 0) { hasValidSize = false; return; }
  ...
  function animate() {
    if (!active) return;
    if (!hasValidSize) { rafId = requestAnimationFrame(animate); return; }
